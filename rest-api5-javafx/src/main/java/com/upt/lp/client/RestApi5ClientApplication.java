...
public class RestApi5ClientApplication {
	...
	// ---------- METODO QUE O DIOGO ADICIONOU -------------------------------------------

	/*
	 * private RestTemplate restTemplate;
	 * 
	 * public RestClient() { this.restTemplate = new RestTemplate(); }
	 */
	...
	private final String rootAPIURLEQUIP = "http://localhost:8080/api/equipamentos";
		...
		try {
			...
			if (response.getStatusCode().is2xxSuccessful()) {
				...
				if (loggedInUser.getUserType() == UserType.DONOR) {
					...
					System.out.println("VOCE SERA DIRECIONADO PARA O MENU DO RECIPIENT ---- TESTE");
	...
	public void searchManager(String email, String password) {
		...
		try {
			...
			Utilizador utilizador = new Utilizador(null, "Carlos", "carlos@gmail.com", "Eucarlos@2001",
					UserType.MANAGER);
	...
	/*
	 * public void getAllEquipamentos() { ResponseEntity<Equipamento[]> response =
	 * restTemplate.getForEntity(rootAPIURLEQUIP, Equipamento[].class);
	 * 
	 * if (response.getStatusCode().is2xxSuccessful()) { Equipamento[] equipamentos
	 * = response.getBody(); if (equipamentos != null) { for (Equipamento
	 * equipamento : equipamentos) { System.out.println(); System.out.println();
	 * System.out.println(equipamento.toString()); } } else {
	 * System.out.println("No body"); } } else {
	 * System.out.println("Nothing found"); } }
	 */
	...
	// --------------ENTENDER O QUE O DIOGO FEZ AQUI
	// ------------------------------------------------------------------
	/*
	 * public void getAllComputadores() {
	 * 
	 * ResponseEntity<Computador[]> response =
	 * restTemplate.getForEntity(rootAPIURLCOMP, Computador[].class);
	 * 
	 * if (response.getStatusCode().is2xxSuccessful()) { Computador[] computadores =
	 * response.getBody(); if (computadores != null) { for (Computador computador :
	 * computadores) { System.out.println(); System.out.println();
	 * System.out.println(computador.toString()); } } else {
	 * System.out.println("No body"); } } else {
	 * System.out.println("Nothing found"); } }
	 */
	// ----------------------------------------------------------------------------------------------------------------

	// ----------COMENTARIOS DO DIOGO (ABAIXO) -----------------------
	/*
	 * public void getAllComputadores() {
	 * 
	 * ResponseEntity<Computador[]> response =
	...
	public List<Equipamento> getAllEquipamentos() {
		ResponseEntity<Equipamento[]> response = restTemplate.getForEntity(rootAPIURLEQUIP, Equipamento[].class);

		List<Equipamento> equipamentos = new ArrayList<>();

		if (response.getStatusCode().is2xxSuccessful()) {
			if (response.getBody() != null) {
				// Adiciona os objetos Equipamento à lista
				for (Equipamento e : response.getBody()) {
					equipamentos.add(e);
				}
				return equipamentos;
			} else {
	 ...
	 * 
	 * System.out.print("\nEnter more information about the accessory: "); String
	 * moreInformation = scanner.nextLine();
	 * acessorio.setInformacao(moreInformation);
	 * 
	 * System.out.println("\nEnter a name to identify the accessory:"); String
	 * accessoryName = scanner.nextLine(); acessorio.setNome(accessoryName);
	 * 
	 * System.out.print("\nEnter the year: "); String accessoryYear =
	 * scanner.nextLine(); acessorio.setAno(accessoryYear);
	 * 
	 * System.out.print("\nEnter accessory brand: "); String accessoryBrand =
	 * scanner.nextLine(); acessorio.setBrand(accessoryBrand);
	 * 
	 * System.out.print("\nEnter the accessory conservation status:");
	 * ConservationStatus accessoryConservation =
	 ...
	 * acessorio.setUserID(loggedInUserId); // Atribuindo o ID do usuário logado ao
	 * acessório
	 * 
	...
	// ---------- ANTIGO METODO CRIAR COMPUTADOR
	// ------------------------------------------------------------------------------------
	/*
	 ...
	 * String computerType = scanner.nextLine(); computador.setType(computerType);
	 * 
	 * System.out.print("\nEnter RAM capacity: (EX: 4GB, 8GB, 16GB, 32GB or more): "
	 * ); String ramCapacity = scanner.nextLine(); computador.setRam(ramCapacity);
	 * 
	 * System.out.
	 * print("\nEnter the drive type: (EX: HDD (Hard Disk Drive) or SSD (Solid State Drive): "
	 * ); String driveType = scanner.nextLine(); computador.setDisco(driveType);
	 * 
	 ...
	 * 
	 * System.out.print("\nEnter the name: "); String computerName =
	 * scanner.nextLine(); computador.setNome(computerName);
	 * 
	 * System.out.print("\nEnter the year: "); String computerYear =
	 * scanner.nextLine(); computador.setAno(computerYear);
	 * 
	 * System.out.
	 ...
	 * ConservationStatus computerConservation =
	 * ConservationStatus.valueOf(scanner.nextLine().toUpperCase());
	 * computador.setEstadoConservacao(computerConservation);
	 * 
	 * // Atribuindo o userID (o ID do usuário logado)
	 * computador.setUserID(loggedInUserId); // Esse é o ponto chave. Preencha com o
	 * ID do usuário
	 * 
	 * ResponseEntity<Computador> response =
	 * restTemplate.postForEntity(rootAPIURLCOMP, computador, Computador.class);
	 * 
	 * if (response.getStatusCode().is2xxSuccessful()) { Computador body =
	 * response.getBody(); if (body != null) { System.out.println(body.toString());
	 * } else { System.out.println("No body"); } } else {
	 * System.out.println("Nothing found"); } }
	 */
	 ...
	 * 
	 * if (!inputAcessoryConservation.isEmpty()) { try { ConservationStatus
	 * newConservation = ConservationStatus.valueOf(inputAcessoryConservation);
	 ...
	 * 
	 * /* ------------(ISSO REALMENTE ESTAVA COMENTADO ANTES)--------------------
	...
	/*
	 * } else { System.out.println("Accessory with ID " + id + " not found."); } }
	 * catch (HttpClientErrorException.NotFound e) {
	 * System.out.println("Accessory with ID " + id +
	 * " not found (404 Not Found)."); } catch (Exception e) {
	 * System.out.println("An error occurred: " + e.getMessage()); } }
	 */
	// ----------------------------------------------------------------------------------------------------------------------------------------

	// -------------NOVO METODO UPDATE ACESSORIO ------------------------------
	...
	// -------------------------------------------------------------------------------------------------------------------------------------------
	// ---------- ANTIGO METODO UPDATE COMPUTADOR
	// ------------------------------------------------------------------------------------
	/*
	 * public void updateComputador(Scanner scanner) {
	 * System.out.print("\nEnter the ID of the computer to update: "); Long id =
	 * scanner.nextLong(); scanner.nextLine();
	 * 
	 * try {
	 * 
	 * ResponseEntity<Computador> response =
	 * restTemplate.getForEntity(rootAPIURLCOMP + "/" + id, Computador.class);
	 * 
	 * if (response.getStatusCode().is2xxSuccessful() && response.getBody() != null)
	 * { Computador computador = response.getBody();
	 * 
	 * System.out.println("Current details of the computer: " +
	 * computador.toString());
	 * 
	 * System.out
	 ...
	 * 
	 * if (!inputConservation.isEmpty()) { try { ConservationStatus newConservation
	 * = ConservationStatus.valueOf(inputConservation);
	 * computador.setEstadoConservacao(newConservation); } catch
	 * (IllegalArgumentException e) {
	 * System.out.println("Estado de conservação inválido. Tente novamente."); } }
	 * 
	 * // ConservationStatus newConservation = //
	 * ConservationStatus.valueOf(scanner.nextLine().toUpperCase()); // if
	 * (!newConservation.isEmpty()) //
	 ...
	 * 
	 * } else { System.out.println("Computer with ID " + id + " not found."); } }
	 * catch (HttpClientErrorException.NotFound e) {
	 * System.out.println("Computer with ID " + id + " not found (404 Not Found).");
	...
	// -------------------------------------------------------------------------------------------------------------------------------------------
	// ---------------------TUDO DAQUI PARA BAIXO NÃO TINHA NA VERSÃO DO DIOGO
	// ----------------------------------------------------------
	/*---------------------------------------------------------------------------------------------------------------inicio
	 ...
	 * -----------------------------------------------------------------------------
	 * --------------------------------- inicio private void
	 * checkDonationRequests(Scanner scanner, Long loggedInUserId) {
	 * 
	 * // Chama o método getDonationRequestsByDonor para obter os pedidos de doação
	 * da // API List<Doacao> donationRequests =
	 * getDonationRequestsByDonor(loggedInUserId);
	 * 
	 * if (donationRequests != null && !donationRequests.isEmpty()) {
	 * System.out.println("\nYou have received the following donation requests:");
	 * 
	 * for (Doacao donation : donationRequests) { System.out.println("Donation ID: "
	 * + donation.getId()); System.out.println("Requestor ID: " +
	 * donation.getIdRequerente()); System.out.println("Donor ID: " +
	 * donation.getIdDoador()); System.out.println("Equipment ID: " +
	 * donation.getIdEquipamento()); System.out.println("Date of the request: " +
	 * donation.getDataInicio());
	 * 
	 * // Pedir a decisão do usuário System.out.println("1: Accept\n2: Reject"); int
	 * choice = scanner.nextInt(); scanner.nextLine(); // Consumir nova linha
	 * 
	 * if (choice == 1) { acceptDonation(donation.getId()); // Aceitar doação } else
	 * { rejectDonation(donation.getId()); // Recusar doação } } } else {
	 * System.out.println("\nNo donation requests at the moment."); } }
	 * 
	 * private void acceptDonation(Long donationId) { // Função para aceitar doação
	 * String url = rootAPIURLDOAC + "/accept-donation/" + donationId; try {
	 * ResponseEntity<String> response = restTemplate.exchange(url, HttpMethod.PUT,
	 * null, String.class); if (response.getStatusCode().is2xxSuccessful()) {
	 * System.out.println("Donation accepted successfully."); } else {
	 * System.out.println("Failed to accept donation."); } } catch (Exception e) {
	 * System.out.println("Error: " + e.getMessage()); } }
	 * 
	 * private void rejectDonation(Long donationId) { // Função para recusar doação
	 * String url = rootAPIURLDOAC + "/reject-donation/" + donationId; try {
	 * ResponseEntity<String> response = restTemplate.exchange(url, HttpMethod.PUT,
	 * null, String.class); if (response.getStatusCode().is2xxSuccessful()) {
	 * System.out.println("Donation rejected successfully."); } else {
	 * System.out.println("Failed to reject donation."); } } catch (Exception e) {
	 * System.out.println("Error: " + e.getMessage()); } }
	 * 
	 * 
	 */ // ---------------------------------------------------------------------------------------------------------------fim
	...
	// MENUUUUUUUUUUU
	/*
	 * private void recipientMenu(Scanner scanner, Long loggedInUserId) { int
	 * option; do { System.out.println("\n===== RECIPIENT MENU =====");
	 ...
	 * choiceEquipmentType = scanner.nextInt(); scanner.nextLine();
	 * 
	 * if (choiceEquipmentType == 1) { // Listar todos os computadores
	 * getAllComputadores(); } else { // Listar todos os acessórios
	 * getAllAcessorios(); } break;
	 * 
	 * case 2: registrarPedidoDoacao(scanner, loggedInUserId); break;
	 * 
	 * case 3:
	 ...
	 * 
	 * case 5: System.out.println("Exiting Recipient Menu..."); break;
	 * 
	 * default: System.out.println("Invalid option. Please try again."); } } while
	 * (option != 5); }
	 * 
	 * private void registrarPedidoDoacao(Scanner scanner, Long loggedInUserId) {
	 * System.out.print("Enter the ID of the equipment you want to request: "); Long
	 * idEquipamento = scanner.nextLong(); scanner.nextLine(); // Consumir a linha
	 * restante
	 * 
	 * System.out.print("Enter the ID of the donor: "); Long idDoador =
	 * scanner.nextLong(); scanner.nextLine();
	 * 
	 * String url = rootAPIURLDOAC + "/registrar" + "?idRequerente=" +
	 * loggedInUserId + "&idDoador=" + idDoador + "&idEquipamento=" + idEquipamento;
	 * 
	 * try { ResponseEntity<Doacao> response = restTemplate.postForEntity(url, null,
	 * Doacao.class);
	 * 
	 * if (response.getStatusCode().is2xxSuccessful()) {
	 * System.out.println("Donation request successfully registered!");
	 * System.out.println(response.getBody()); } else {
	 * System.out.println("Failed to register donation request."); } } catch
	 * (Exception e) { System.out.println("Error: " + e.getMessage()); } }
	 * 
	 * private void getAllDonationsByRequesterId(Long idRequerente) { String url =
	 * rootAPIURLDOAC + "/all-by-requester-id?idRequerente=" + idRequerente;
	 * 
	 * try { ResponseEntity<Doacao[]> response = restTemplate.getForEntity(url,
	 * Doacao[].class);
	 * 
	 * if (response.getStatusCode().is2xxSuccessful() && response.getBody() != null)
	 * { Doacao[] donations = response.getBody();
	 * System.out.println("\nDonations associated with requester ID " + idRequerente
	 * + ":"); for (Doacao doacao : donations) { System.out.println(doacao); } }
	 * else {
	 ...
	 * ); System.out.println("1: Finish Donation\n2: Cancel Donation"); int action =
	 * scanner.nextInt(); scanner.nextLine();
	 * 
	 * String actionUrl = (action == 1) ? "/finalizar/" : "/cancelar/"; String url =
	 * rootAPIURLDOAC + actionUrl + idDoacao;
	 * 
	 * try { ResponseEntity<Doacao> response = restTemplate.exchange(url,
	 * HttpMethod.PUT, null, Doacao.class);
	 * 
	 * if (response.getStatusCode().is2xxSuccessful() && response.getBody() != null)
	 * { System.out .println((action == 1 ? "Donation successfully finished!" :
	 * "Donation successfully canceled!")); System.out.println(response.getBody());
	 * } else { System.out.println("Failed to update the donation status."); } }
	 * catch (Exception e) { System.out.println("Error: " + e.getMessage()); } }
	 */ // ---------------------------------------------------------------------------------------------------------------fim
