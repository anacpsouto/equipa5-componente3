...
public class RestApi5ClientApplication {
	...
	// alterei para incluir o menu administrador
	...
	private void login(Scanner scanner) {
	    ...
	    try {
	        ...
	        if (response.getStatusCode().is2xxSuccessful()) {
	            ...
	            } else if ("ADMINISTRATOR".equalsIgnoreCase(loggedInUser.getUserType())) {
	                admininMenu(scanner, loggedInUserId);
	            } else {
	...
	// criei administrator menu
	
		private void admininMenu(Scanner scanner, Long loggedInUserId) {
			
			while (true) {
				
				System.out.println("\n===== ADMINISTRATOR MENU =====");
				  System.out.println("1. Manage donations (to cancel them or change something)");
				  ...
				  switch (option) {
				  	...
				  	case 1:
				  		manageDonations(scanner, loggedInUserId);
					
				  	case 2:
				  		manageEquipments(scanner, loggedInUserId);
				  	
				  	case 3:
				  		manageUsers(scanner, loggedInUserId);
				  	
				  	case 4:
				  		viewStatistics(scanner, loggedInUserId);
		...
		private void manageUsers(Scanner scanner, Long loggedInUserId) {
			System.out.println("\n===== MANAGE USERS =====");
		...
		private void manageEquipments(Scanner scanner) {
		    System.out.println("\n===== MANAGE EQUIPMENTS =====");
		    System.out.println("1. View Equipments To Review");
		...
		private void getAllEquipmentsReview() {
		    // Searchr computadores pendentes
		    List<Computador> computadoresPendentes = computadorService.findByEstado(EstadoEquipamento.PENDENTE);
		    // Busca acessórios pendentes
		    List<Acessorio> acessoriosPendentes = acessorioService.findByEstado(EstadoEquipamento.PENDENTE);

		    // Verifica se há equipamentos pendentes
		    if (computadoresPendentes.isEmpty() && acessoriosPendentes.isEmpty()) {
		        System.out.println("\nNo equipaments with pending review.");
		        return;
		    }

		    System.out.println("\n===== EQUIPMENTS TO REVIEW =====");

		    // Exibe os computadores pendentes
		    if (!computadoresPendentes.isEmpty()) {
		        System.out.println("\n--- COMPUTERS ---");
		        for (Computador computador : computadoresPendentes) {
		            System.out.println(computador.toString());
		        }
		    }

		    // Exibe os acessórios pendentes
		    if (!acessoriosPendentes.isEmpty()) {
		        System.out.println("\n--- ACESSORIES ---");
		        for (Acessorio acessorio : acessoriosPendentes) {
		            System.out.println(acessorio.toString());
		        }
		    }

		    // Review equipamentos
		    reviewEquipments(computadoresPendentes, acessoriosPendentes);
		}
		
		
		// depois de criar a tabela marcas, se a brand que o utilizador pos nao estiver na tabela, adicionar opcao para o admin adicionar a marca à tabela
		private void reviewEquipments(List<Computador> computadoresPendentes, List<Acessorio> acessoriosPendentes) {
		    Scanner scanner = new Scanner(System.in);

		    // Review de computadores
		    for (Computador computador : computadoresPendentes) {
		        System.out.println("\nReviewing Computer:");

		        // Verificar se há violação de regra
		        if (Integer.parseInt(computador.getAno()) < 2012) {
		            System.out.println("Breaking Rule : Equipament bought before 2012.");
		            System.out.println("\nWhat do you wish to do with this computer?");
		            System.out.println("1. Refuse");
		            System.out.println("2. Review next computer");
		            ...
		            switch (escolha) {
		                case 1:
		                    computador.setEstadoEquipamento(EstadoEquipamento.RECUSADO);
		                    computadorService.updateComputadorAdmin(computador);
		                    System.out.println("Computer refused with sucess!");
		                    break;
		                case 2:
		                    System.out.println("Moving to next computer...");
		                    break;
		                default:
		                    System.out.println("Invalid option. Moving to next computer...");
		            }
		        } else {
		            // Mostrar informações do computador se estiver dentro das regras
		            System.out.println(computador.toString());
		            ...
		            System.out.println("3. Review next computer");
		            ...
		            switch (escolha) {
		                case 1:
		                    computador.setEstadoEquipamento(EstadoEquipamento.ACEITE);
		                    computadorService.updateComputadorAdmin(computador);
		                    System.out.println("Computer acepted with sucess!");
		                    break;
		                case 2:
		                    computador.setEstadoEquipamento(EstadoEquipamento.RECUSADO);
		                    computadorService.updateComputadorAdmin(computador);
		    ...
		    // Review de acessórios
		    for (Acessorio acessorio : acessoriosPendentes) {
		        System.out.println("\nReviewing Accessory:");

		        // Verificar se há violação de regra
		        if (Integer.parseInt(acessorio.getAno()) < 2012) {
		            System.out.println("Breaking Rule: Equipment bought before 2012.");
		            System.out.println("\nWhat do you wish to do with this accessory?");
		            System.out.println("1. Refuse");
		            System.out.println("2. Review next accessory");
		            ...
		            switch (escolha) {
		                case 1:
		                    acessorio.setEstadoEquipamento(EstadoEquipamento.RECUSADO);
		                    acessorioService.updateAcessorioAdmin(acessorio);
		                    System.out.println("Accessory refused with success!");
		                    break;
		                case 2:
		                    System.out.println("Moving to next accessory...");
		                    break;
		                default:
		                    System.out.println("Invalid option. Moving to next accessory...");
		            }
		        } else {
		            // Mostrar informações do acessório se estiver dentro das regras
		            System.out.println(acessorio.toString());
		            ...
		            System.out.println("1. Accept");
		            System.out.println("2. Refuse");
		            System.out.println("3. Review next accessory");
		            ...
		            switch (escolha) {
		                case 1:
		                    acessorio.setEstadoEquipamento(EstadoEquipamento.ACEITE);
		                    acessorioService.updateAcessorioAdmin(acessorio);
		                    System.out.println("Accessory accepted with success!");
		                    break;
		                case 2:
		                    acessorio.setEstadoEquipamento(EstadoEquipamento.RECUSADO);
		                    acessorioService.updateAcessorioAdmin(acessorio);
